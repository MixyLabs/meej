...
func newPASession(
	...
	// use a self-identifying session name e.g. meej.sessions.chrome
...
func (s *paSession) InternalKey() string {
	if s.system {
		return systemSessionName
	}

	return strconv.Itoa(int(s.sinkInputIndex))
}
...
/*func (s *masterSession) InternalKey() string {
	if s.system {
		return systemSessionName
	}

	return strconv.Itoa(int(s.streamIndex))
}*/
