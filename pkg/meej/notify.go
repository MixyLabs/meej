...
func (tn *ToastNotifier) Notify(title string, message string) {
	// we need to unpack meej.ico somewhere to remain portable. we already have it as bytes so it should be fine
	appIconPath := filepath.Join(os.TempDir(), "meej.ico")

	if !util.FileExists(appIconPath) {
		tn.logger.Debugw("Meej icon file missing, creating", "path", appIconPath)
		...
		if _, err = f.Write(MeejLogoIconData); err != nil {
